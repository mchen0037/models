...
to infect-others-around-me
  if color = red [
    ask turtles-on neighbors4  [
      ; if masks are mandated then we have a 25% chance of infecting
      ifelse wear-masks? [
        if random 100 < 25 [
          set color red
        ]
...
to-report get-day
  ; it takes about 2 minutes to get from me to the bathroom at a walking pace
  ; it's aout 180 netlogo steps to get from rm 26 to the bathroom
  ; ~180 netlogo steps/tick (each turtle takes 1 step per tick) is about 2 minutes
  ; ~5400 ticks to an hour
  ; ~43000 ticks in a day
  report ticks mod 43000
end
